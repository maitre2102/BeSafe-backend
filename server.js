...
const frontendUrl = process.env.FRONTEND_URL || 'https://besafe-frontend.onrender.com';
...
app.post('/api/alerts', async (req, res) => {
  const { userName, latitude, longitude, contacts, incidentId } = req.body;
  ...
  const trackLink = incidentId
    ? `${frontendUrl}/track.html?id=${incidentId}`
    : ((latitude != null && longitude != null) ? `https://maps.google.com/?q=${latitude},${longitude}` : null);

  const message = trackLink
    ? `${userName || 'Une personne'} a besoin d'une assistance d'urgence. Suis sa position en direct ici : ${trackLink}`
    : `${userName || 'Une personne'} a besoin d'une assistance d'urgence. Position indisponible.`;
...
app.post('/api/soft-alert', async (req, res) => {
  const { userName, contactPhone, latitude, longitude, incidentId } = req.body;
  ...
  const message = trackLink
    ? `${userName || 'Une personne'} se trouve dans une situation inconfortable. Veuillez garder un œil sur sa position en direct ici : ${trackLink}`
    : `${userName || 'Une personne'} se trouve dans une situation inconfortable. Position indisponible.`;
...
// ---------------------------------------------------------------------------
// Suivi en direct, ouvert depuis le lien envoyé par SMS à une personne de confiance.
// Volontairement public (pas de clé admin) : le lien lui-même, imprévisible et
// propre à un seul incident, fait office de laissez-passer — comme un lien de
// partage de position classique. On ne renvoie que le strict nécessaire : ni
// numéro de téléphone, ni identifiant d'appareil, ni autres incidents.
// ---------------------------------------------------------------------------
app.get('/api/incidents/:id/track', async (req, res) => {
  if (!pool) return res.status(503).json({ error: 'Suivi indisponible (pas de base de données configurée).' });
  try {
    const incident = await pool.query(
      'SELECT id, user_name, type, started_at, ended_at FROM incidents WHERE id = $1',
      [req.params.id]
    );
    if (!incident.rows.length) return res.status(404).json({ error: 'Lien de suivi invalide ou expiré.' });

    const positions = await pool.query(
      'SELECT latitude, longitude, recorded_at FROM incident_positions WHERE incident_id = $1 ORDER BY recorded_at ASC',
      [req.params.id]
    );

    res.json({
      userName: incident.rows[0].user_name,
      type: incident.rows[0].type,
      startedAt: incident.rows[0].started_at,
      endedAt: incident.rows[0].ended_at,
...
function requireAdmin(req, res, next) {
  ...
  const match = !!ADMIN_KEY && key === ADMIN_KEY;
  console.log(`Vérif admin — clé reçue: ${key ? key.length + ' caractères' : 'aucune'}, clé attendue: ${ADMIN_KEY.length} caractères, correspondance: ${match}`);
